      ...
      <SafeAreaView className="flex-1 bg-black/70">
        <View className="flex-1 px-6 pt-8">
          ...
          <View className="bg-white/10 rounded-xl p-6 border border-pink-400/30 mb-6">
            ...
            {user?.role === 'reader' && (
              <View className="mt-4">
                <Text className="text-gold-400">Rating: {(user as any)?.rating || 'N/A'} ⭐</Text>
                <Text className="text-green-400">Total Earnings: ${(user as any)?.totalEarnings?.toFixed(2) || '0.00'}</Text>
              </View>
            )}
          </View>
